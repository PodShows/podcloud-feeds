...
deploy() {
	...
	if [ "$deploy_env" = "staging" ]; then
		...
		DEPLOY_HOST="podcloud@eve.podradio.fr"
		DEPLOY_DIR="/home/podcloud/production/feeds"
	fi

	ssh-keyscan $DEPLOY_HOST >> ~/.ssh/known_hosts

	ssh $DEPLOY_HOST /bin/bash bash -c "mkdir -p $DEPLOY_DIR"

	rsync -avzPL --delete-after $DIR/ $DEPLOY_HOST:$DEPLOY_DIR
	...
	ssh $DEPLOY_HOST /bin/bash -c "${CMD}"
}
