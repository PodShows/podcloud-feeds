...
module.exports = {
  ...
  deploy: {
    production: {
      ...
      "post-deploy":
        "npm install && NODE_ENV=production pm2 startOrReload ecosystem.config.js --env production",
      env: {
        NODE_ENV: "production"
      }
    },
    preprod: {
      ...
      "pre-deploy-local": "ssh-keyscan eve.podradio.fr >> ~/.ssh/known_hosts",
      "post-deploy":
        "npm install && NODE_ENV=preprod pm2 startOrReload ecosystem.config.js --env preprod",
      env: {
        NODE_ENV: "preprod"
      }
    }
